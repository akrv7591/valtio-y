...
describe("Integration: Deep Nesting (Property-Based)", () => {
    ...
    it("should sync any deeply nested object structure to Y.js", async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (structure) => {
            const { proxy, doc } = createDocWithProxy<LooseRecord>((d) =>
              d.getMap("root"),
            );
            ...
            const yJSON = yData
              ? (yData as Y.Map<unknown> | Y.Array<unknown>).toJSON()
              : yData;
        ...
        fc.asyncProperty(
          fc.array(
            fc.oneof(
              ...
              fc.record({
                id: fc.integer(),
                nested: fc.array(fc.string(), { maxLength: 5 }),
              }),
            ),
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async ({ initial, mutations }) => {
            const { proxyA, proxyB, bootstrapA } =
              createRelayedProxiesMapRoot();
            ...
            expect(JSON.stringify(proxyB.data)).toEqual(
            ...
            for (const { key, value } of mutations) {
              if (
                proxyA.data &&
                typeof proxyA.data === "object" &&
                !Array.isArray(proxyA.data)
              ) {
            ...
            expect(JSON.stringify(proxyA.data)).toEqual(
              JSON.stringify(proxyB.data),
            );
          },
        ),
  ...
  describe("Performance Properties", () => {
    it("should complete operations in reasonable time regardless of depth", async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer({ min: 5, max: 15 }), async (depth) => {
          const { proxy } = createDocWithProxy<LooseRecord>((d) =>
            d.getMap("root"),
          );
    ...
    it("should handle wide objects efficiently", async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer({ min: 50, max: 500 }), async (numKeys) => {
          const { proxy } = createDocWithProxy<LooseRecord>((d) =>
            d.getMap("root"),
          );
  ...
  describe("Concrete Deep Structure Edge Cases", () => {
    it("should handle tree structure with 1000 nodes", async () => {
      ...
      const deepNode =
        proxy.tree.children[0].children[0].children[0].children[0].children[0];
      ...
      proxy.tree.children[1].children.push({
        id: 9999,
        value: "new-node",
        isLeaf: false,
      });
      await waitMicrotask();
      expect(
        proxy.tree.children[1].children[
          proxy.tree.children[1].children.length - 1
        ].value,
      ).toBe("new-node");
    });
    ...
    it("should handle deletion in deep structure", async () => {
      ...
      proxy.data = {
        l1: {
          l2: { l3: { l4: { l5: { value: "deep", toDelete: "remove me" } } } },
        },
      };
      await waitMicrotask();
