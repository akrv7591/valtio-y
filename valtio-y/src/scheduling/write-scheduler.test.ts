...
describe("Conservative merge check - detailed analysis", () => {
  it("logs Y.js operations for single assignment (should merge)", async () => {
    ...
    const { proxy, dispose, bootstrap } = createYjsProxy<{ items: unknown[] }>(
      doc,
      {
  ...
  it("compares Y.js structure for single vs multiple assignments", async () => {
    ...
    } = createYjsProxy<{ items: unknown[] }>(doc1, {
      getRoot: (doc) => doc.getMap("root"),
    });
    ...
    console.log(
      "Single assignment - First item type:",
      yItems1.get(0)?.constructor.name,
    );
    ...
    } = createYjsProxy<{ items: unknown[] }>(doc2, {
      getRoot: (doc) => doc.getMap("root"),
    });
    ...
    console.log(
      "Multiple assignments - First item type:",
      yItems2.get(0)?.constructor.name,
    );
    console.log(
      "Multiple assignments - Third item type:",
      yItems2.get(2)?.constructor.name,
    );
  ...
  it("checks if identity is preserved with single assignment", async () => {
    ...
    const originalId = (
      originalItem as YTypeWithInternals
    )._item?.id?.toString();
  ...
  it("checks if identity is preserved with multiple assignments", async () => {
    ...
    const originalId0 = (
      originalItem0 as YTypeWithInternals
    )._item?.id?.toString();
    const originalId2 = (
      originalItem2 as YTypeWithInternals
    )._item?.id?.toString();
